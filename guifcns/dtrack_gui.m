...
gui.infoarea.info.tab = uitab(gui.infoarea.info.tabgroup, 'title', 'Info'); 
    gui.infoarea.info.entries.framenr = uicontrol(gui.infoarea.info.tab, 'style', 'edit', 'string', ['frame ' num2str(status.framenr) '/' num2str(status.mh.NFrames)], 'units', 'normalized', 'position', [0 .5 .5 .5], 'tag', 'framenr', 'enable', 'inactive', 'buttondownfcn', status.maincb);
    fth = floor(status.framenr/status.mh.FrameRate/3600);
    ftm = floor(mod(status.framenr/status.mh.FrameRate/60,3600));
    fts = floor(mod(status.framenr/status.mh.FrameRate,60));
    ftms = mod(status.framenr/status.mh.FrameRate, 1)*1000;
