...
set(gui.f1, 'outerposition', fig1pos, 'name', ['DTrack: ' para.paths.resname ' (' para.paths.movname ')'], ...
...
gui.infoarea.info.panel=uipanel(gui.f1, 'position', [.85 .94 .15 .06]);
    ...
    if ~isempty(para.ref.framenr)
        gui.infoarea.info.entries.refframe=uicontrol(gui.infoarea.info.panel, 'style', 'edit', 'string', ['ref frame ' num2str(para.ref.framenr)], 'units', 'normalized', 'position', [.5 0 .5 .5], 'tag', 'refframe', 'enable', 'inactive', 'buttondownfcn', status.maincb);
    else
        gui.infoarea.info.entries.refframe=uicontrol(gui.infoarea.info.panel, 'style', 'edit', 'string', 'ref frame not set', 'units', 'normalized', 'position', [.5 0 .5 .5], 'tag', 'refframe', 'enable', 'inactive', 'buttondownfcn', status.maincb);      
    end
...
gui.menus.roi.menu = uimenu(gui.f1, 'label', 'RoI&&Ref');
    ...
    gui.menus.roi.entries.ref_set = uimenu(gui.menus.roi.menu, 'label', 'Set reference frame', 'separator', 'on');
    gui.menus.roi.entries.ref_clear = uimenu(gui.menus.roi.menu, 'label', 'Clear reference frame');
    gui.menus.roi.entries.ref_display = uimenu(gui.menus.roi.menu, 'label', 'Use reference frame', 'checked', 'off');
    
%% Tools menu
gui.menus.tools.menu = uimenu(gui.f1, 'label', 'Tools');
    gui.menus.tools.entries.tools_imageone_jpg = uimenu(gui.menus.tools.menu, 'label', 'Save frame as jpg', 'accelerator', 'j');
    gui.menus.tools.entries.tools_imageone_tif = uimenu(gui.menus.tools.menu, 'label', 'Save frame as tif');
    gui.menus.tools.entries.tools_imageseq = uimenu(gui.menus.tools.menu, 'label', 'Save as image sequence...');
